...
public class Factura implements Calculo, Serializable {
    ...
    public Object[][] getArrayDetalleFactura() {
        //Pasajero 
        Object[][] array = new Object[this._detalleFactura.size()][5];
        for (int i = 0; i < _detalleFactura.size(); i++) {
            array[i][0] = _detalleFactura.get(i).get_boleto().get_pasajero().get_nombre() + " " + _detalleFactura.get(i).get_boleto().get_pasajero().get_apellido();
            array[i][1] = _detalleFactura.get(i).get_boleto().get_origen().get_descripcion() + " // " + _detalleFactura.get(i).get_boleto().get_destino().get_descripcion();
            array[i][2] = _detalleFactura.get(i).get_boleto().get_categoriaVuelo().get_descripcion();
            array[i][3] = _detalleFactura.get(i).get_boleto().get_asiento().get_codigo();
            array[i][4] = _detalleFactura.get(i).get_subtotal();
